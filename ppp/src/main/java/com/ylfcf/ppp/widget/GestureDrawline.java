...
import com.ylfcf.ppp.entity.GesturePoint;
import com.ylfcf.ppp.util.Constants.TouchType;
...
public class GestureDrawline extends View{
	...
	public GestureDrawline(Context context, List<GesturePoint> list, boolean isVerify,
		...
		super(context);
		screenDispaly = SettingsManager.getScreenDispaly(context);
	...
	public boolean onTouchEvent(MotionEvent event) {
		...
		case MotionEvent.ACTION_DOWN:
			...
			if (currentPoint != null) {
				currentPoint.setPointState(TouchType.POINT_STATE_SELECTED);
		...
		case MotionEvent.ACTION_MOVE:
			...
			if (pointAt == null || currentPoint.equals(pointAt) || TouchType.POINT_STATE_SELECTED == pointAt.getPointState()) {
				...
				pointAt.setPointState(TouchType.POINT_STATE_SELECTED);
				
				// 判断是否中间点需要选中
				GesturePoint betweenPoint = getBetweenCheckPoint(currentPoint, pointAt);
				if (betweenPoint != null && TouchType.POINT_STATE_SELECTED != betweenPoint.getPointState()) {
					// 存在中间点并且没有被选中
					...
					// 设置中间点选中
					betweenPoint.setPointState(TouchType.POINT_STATE_SELECTED);
					// 赋值当前的point;
					currentPoint = pointAt;
				} else {
	...
	final class clearStateRunnable implements Runnable {
		public void run() {
			...
			for (GesturePoint p : list) {
				p.setPointState(TouchType.POINT_STATE_NORMAL);
			}
			invalidate();
			isDrawEnable = true;
		}
	...
	private void drawErrorPathTip() {
		...
		for (Pair<GesturePoint, GesturePoint> pair : lineList) {
			pair.first.setPointState(TouchType.POINT_STATE_WRONG);
			pair.second.setPointState(TouchType.POINT_STATE_WRONG);
			canvas.drawLine(pair.first.getCenterX(), pair.first.getCenterY(),
					pair.second.getCenterX(), pair.second.getCenterY(), paint);// 画线
		}
		invalidate();
