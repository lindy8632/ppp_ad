...
public class PagerSlidingTabStrip extends HorizontalScrollView{
	...
	private int indicatorColor = 0xFF31B2FE;//底部的短线
	private int underlineColor = 0xFFe5e5e5;//底部的长线
	private int dividerColor = 0x00000000;//每个导航栏之间的分割线
	...
	private int selectedTabTextColor = 0xFF31B2FE;
	private Typeface tabTypeface = null;
	private int tabTypefaceStyle = Typeface.NORMAL;

	private int lastScrollX = 0;

	private int tabBackgroundResId = R.drawable.slid_tab_background;
	...
	public void notifyDataSetChanged() {
		...
		for (int i = 0; i < tabCount; i++) {
			...
			if (pager.getAdapter() instanceof IconTabProvider) {
				addIconTab(tabCount,i,
						((IconTabProvider) pager.getAdapter())
								.getPageIconResId(i));
			} else {
				addTextTab(tabCount,i, pager.getAdapter().getPageTitle(i).toString());
			}
	 ...
	 * @param title
	 */
	private void addTextTab(int count,final int position, String title) {
		TextView tab = null;
		TextView tab2 = null;
		...
		tab2.setSingleLine();
		addTab(count,position, tab, tab2);
	}

	private void addIconTab(int count,final int position, int resId) {

		ImageButton tab = new ImageButton(getContext());
		tab.setImageResource(resId);

		addTab(count,position, tab, null);

	}

	private void addTab(int count,final int position, View tab, View tab2) {
		...
		if(count <= 3){
			shouldExpand = true;
			framelayout.addView(tab, 0, expandedTabLayoutParams);
			framelayout.addView(tab2, 1, expandedTabLayoutParams);
		}else{
			shouldExpand = false;
